...
Config: copy `config/settings.example.toml` → `config/settings.toml` (camera index, Lichess round ID, PGN player/event headers) and `.env.example` → `.env` (`LICHESS_API_TOKEN`, needs the `study:write` scope). Both copies are gitignored.
