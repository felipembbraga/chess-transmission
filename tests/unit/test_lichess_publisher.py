...
def test_push_sends_pgn_to_configured_round(mocker):
    mock_client_cls = mocker.patch("chess_transmission.publish.lichess_broadcast.berserk.Client")
    mock_client = mock_client_cls.return_value

    publisher = LichessBroadcastPublisher(token="test-token", round_id="round123")
    publisher.push("1. e4 e5 *")

    mock_client.broadcasts.push_pgn_update.assert_called_once_with("round123", ["1. e4 e5 *"])
...
def test_push_raises_after_exhausting_retries(mocker):
    ...
    publisher = LichessBroadcastPublisher(token="test-token", round_id="round123", max_attempts=3)
